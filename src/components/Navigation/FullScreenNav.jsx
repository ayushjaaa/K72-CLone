...
const FullScreenNav = () => {
  ...
  return (
    <div
      ref={fullScreenRef}
      id="fullscreennav"
      className="fullscreennav hidden text-white overflow-hidden h-screen w-full z-50 absolute top-0 left-0"
    >
      {/* 🔹 Background stair animation */}
      ...
      <div ref={fullNavLinksRef} className="relative z-10">
        ...
        {/* 🔹 Animated nav links */}
        <div className="py-36">
          {["Projets", "Agence", "Contact", "Blogs"].map((title, i) => (
            <div
              key={i}
              className={`link origin-top relative border-t ${
                i === 3 ? "border-y" : ""
              } border-white opacity-0 rotate-x-90`}
            >
              <h1 className="font-[font2] text-5xl lg:text-[8vw] text-center lg:leading-[0.8] lg:pt-10 pt-3 uppercase">
                {title}
              </h1>

              {/* Hover moving overlay */}
